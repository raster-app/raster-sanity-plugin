...
function Browser({
		...
		return (
			<Flex direction="column" align="flex-start" gap={4} padding={4}>
				<Card tone="critical" padding={3} radius={2} border>
					<Text size={1}>
						This Raster account can't reach the organization <code>{pinned}</code> that this studio
						is configured for.
					</Text>
				</Card>
				<Button
	...
	const errorMessage =
		actions.error ?? (uploadState.status === 'error' ? uploadState.message : null)
	const progress =
			...
			>
				...
				{search.active ? (
					<AssetGrid
						...
						badgeFor={nav.library === null ? (item) => libraryName(item.libraryId) : undefined}
